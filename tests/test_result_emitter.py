...
from neuropilot.results import ResultEmitter, create_result
...
class TestResultManager(unittest.TestCase):
    ...
    def test_invalid_output_pattern_missing_job_id(self):
        with self.assertRaises(ValueError):
            ResultEmitter(self.tmpdir, output_pattern="{dataset_name}.json")

    def test_invalid_output_pattern_with_unknown_key(self):
        with self.assertRaises(ValueError):
            ResultEmitter(self.tmpdir, output_pattern="{unknown_key}/{job_id}.json")

    def test_emit_and_collect_json_result(self):
        mgr = ResultEmitter(self.tmpdir, output_pattern="{task_name}/{dataset_name}/{job_id}.json", fmt="json")
        result_data = {"acc": 0.9}
    ...
    def test_emit_and_collect_yaml_result(self):
        mgr = ResultEmitter(self.tmpdir, output_pattern="{job_id}.yaml", fmt="yaml")
    ...
    def test_collect_without_inference(self):
        mgr = ResultEmitter(self.tmpdir, output_pattern="{task_name}/{dataset_name}/{job_id}.json")
        mgr.create_and_emit_result(self.job, results={"val": 1.0})

        collected = mgr.collect(infer_metadata=False)
    ...
    def test_collect_with_custom_root_dir_skips_inference(self):
        ...
        collected = mgr.collect(root_dir=alt_root, infer_metadata=True)
        self.assertEqual(len(collected), 0) # No results found because root was changed to an empty one
        
        # Same here, no results
        collected = mgr.collect(root_dir=alt_root, infer_metadata=False)
        self.assertEqual(len(collected), 0) # No results found because root was changed to an empty one

        mgr = ResultEmitter(alt_root, output_pattern="{task_name}/{dataset_name}/{job_id}.json")
        mgr.create_and_emit_result(self.job, results={"acc": 0.8})

        collected = mgr.collect(root_dir=self.tmpdir, infer_metadata=True) # Should be able to find since they're from the previous
    ...
    def test_infer_metadata_failure_due_to_path_mismatch(self):
        ...
        results = mgr.collect(strict=False)
        ...
        results_strict = mgr.collect(strict=True)
    ...
    def test_emit_result_with_custom_object(self):
        mgr = ResultEmitter(self.tmpdir, output_pattern="{job_id}.json")
        custom_result = {
            "id": "abc999",
...
class TestResultManagerPaths(unittest.TestCase):
    ...
    def test_basic_output_path_resolution(self):
        mgr = ResultEmitter(root_dir=self.root, output_pattern="{job_id}.json")
    ...
    def test_missing_fields_use_fallbacks(self):
        ...
        mgr = ResultEmitter(root_dir=self.root, output_pattern="{task_name}/{dataset_name}/{job_id}.json")
    ...
    def test_invalid_output_pattern_rejected(self):
        with self.assertRaises(ValueError) as ctx:
            ResultEmitter(root_dir=self.root, output_pattern="{job_id}/{bad_field}.json")
        self.assertIn("unknown placeholder", str(ctx.exception))

    def test_job_id_required_in_output_pattern(self):
        with self.assertRaises(ValueError) as ctx:
            ResultEmitter(root_dir=self.root, output_pattern="{task_name}.json")
